...
void Utility::fadeOutMusicBasedOnPosition(float playerX, float startX, float endX) {
    static int initialVolume = Mix_VolumeMusic(-1);
    float normalizedPosition = (playerX - startX) / (endX - startX);
    normalizedPosition = std::min(std::max(normalizedPosition, 0.0f), 1.0f);

    int newVolume = static_cast<int>(initialVolume * (1.0f - normalizedPosition));
