...
void Effects::start(EffectType effect_type, float effect_speed)
{
    ...
    switch (m_current_effect)
    {
        ...
        case SHAKE:   m_time_left = 0.2f / effect_speed; break;
    }
}
...
void Effects::update(float delta_time)
{
    switch (m_current_effect)
    {
    case NONE: m_view_offset = glm::vec3(0, 0, 0); break;
    ...
    case SHAKE:
        if (m_time_left <= 0) m_current_effect = NONE;
        m_time_left -= delta_time;
        m_view_offset = glm::vec3(rand() % 2 / 10.0f, rand() % 2 / 10.0f, rand() % 2 / 10.0f); break;
    }
}
